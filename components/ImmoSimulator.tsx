...
export default function ImmoSimulator() {
  ...
  // --- STATES ---
  const [capacity, setCapacity] = useState({ income: 0, charges: 0, duration: 25, rate: 3.8 });
  const [capResults, setCapResults] = useState({ maxLoan: 0, maxMonthly: 0, totalEnvelope: 0 });

  const [project, setProject] = useState({ price: 0, works: 0, contribution: 0, notaryRate: 8, rent: 0, charges: 0, tax: 0 });
  const [projResults, setProjResults] = useState({ totalCost: 0, cashflow: 0, yieldNet: 0, monthlyPayment: 0 });
  ...
  useEffect(() => {
    ...
    let maxLoan = 0;
    if (rateM > 0 && limit > 0) maxLoan = (limit * (1 - Math.pow(1 + rateM, -months))) / rateM;
    
    setCapResults({ maxMonthly: limit, maxLoan: maxLoan, totalEnvelope: maxLoan });
  }, [capacity]);
  ...
  useEffect(() => {
    ...
    let payment = 0;
    if (loan > 0) payment = loan * (rateProj * Math.pow(1 + rateProj, 240)) / (Math.pow(1 + rateProj, 240) - 1);
    ...
    setProjResults({ totalCost: total, cashflow, yieldNet: yieldN, monthlyPayment: payment });
  }, [project]);
