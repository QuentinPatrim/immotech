...
export default function NexusStudio() {
  ...
  const renderVisualPage = (pageIndex: number, refToAttach: any = null) => {
    ...
    return (
        <div 
            ref={refToAttach}
            className={`w-[540px] ${isSingle ? 'min-h-[675px] h-auto pb-12' : 'h-[540px]'} bg-[#08080A] flex flex-col shrink-0 shadow-2xl border border-white/10 font-sans overflow-hidden relative text-white`}
        >
            ...
            <div className={`flex-1 px-8 ${isSingle ? 'py-6 gap-6' : 'py-3 gap-2'} relative z-10 flex flex-col justify-center`}>
                ...
                {(isSingle || pageIndex === 0) && (
                    <div className="grid grid-cols-3 gap-2">
                        {[{ v: data.kpi_1_value, l: data.kpi_1_label }, { v: data.kpi_2_value, l: data.kpi_2_label }, { v: data.kpi_3_value, l: data.kpi_3_label }].map((k, i) => (
                            <div key={i} className={`bg-white/5 rounded-lg p-3 text-center border ${glassEffect ? 'backdrop-blur-md border-white/10' : 'border-white/5'}`}>
                                <p className={`text-lg font-black ${T.text}`}>{k.v}</p>
                                <p className="text-[7px] text-zinc-400 font-bold uppercase tracking-wider">{k.l}</p>
                            </div>
                        ))}
                    </div>
                )}
                        ...
                        <div className="space-y-1">
                            ...
                            <div className={`bg-white/5 border rounded-xl p-3 space-y-2 ${glassEffect ? 'backdrop-blur-sm border-white/10' : 'border-white/5'}`}>
                                {data.analysis_points?.map((pt: string, i: number) => (
                                    <div key={i} className="flex items-start gap-2"><div className={`w-1 h-1 rounded-full ${T.bg} mt-1.5 shrink-0`}></div><p className="text-[10px] text-zinc-300 leading-snug">{pt}</p></div>
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 h-full flex flex-col justify-center">
                        ...
                        {data.show_chart && (
                            <div className={`bg-zinc-900/50 border rounded-xl p-4 flex flex-col relative overflow-hidden ${glassEffect ? 'backdrop-blur-sm border-white/10' : 'border-white/5'} ${isSingle ? 'h-[200px]' : 'flex-1'}`}>
                                <h3 className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-2"><TrendingUp size={10}/> {data.chart_title}</h3>
                ...
                {(isSingle || pageIndex === 3) && (
                    <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
                        ...
                        <div className={`mt-8 px-6 py-3 rounded-full border ${T.border} ${T.text} font-bold text-xs uppercase tracking-widest`}>Téléchargez Nexus</div>
                    </div>
                )}
            </div>
        ...
        <div className="flex-1 lg:overflow-y-auto pr-0 lg:pr-2 space-y-6 scrollbar-hide">
            ...
            {activeTab === "editor" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 pb-20">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-xs font-black text-white uppercase bg-zinc-800 px-3 py-1 rounded w-fit">Preuve</h3>
                            ...
                            <div className="space-y-2 bg-black/20 p-2 rounded">
                                {data.chart_data.map((pt: any, i: number) => (
                                    <div key={i} className="flex gap-2">
                                        <Input value={pt.name} onChange={(e) => updateChartData(i, 'name', e.target.value)} className="w-12 bg-zinc-900 border-white/10 text-xs px-1"/>
                                        <Input type="number" value={pt.value1} onChange={(e) => updateChartData(i, 'value1', Number(e.target.value))} className="bg-zinc-900 border-white/10 text-xs px-1"/>
                                        <Input type="number" value={pt.value2} onChange={(e) => updateChartData(i, 'value2', Number(e.target.value))} className="bg-zinc-900 border-white/10 text-xs px-1"/>
                                    </div>
                                ))}
                            </div>
                        )}
