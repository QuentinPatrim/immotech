...
export default function BrochureClient() {
    ...
    // Récupération des paramètres modifiés par l'agent depuis l'URL
    const sellingPriceFAI = Number(searchParams.get("price")) || 0;
    const agencyFees = Number(searchParams.get("fees")) || 0;
    const feeType = searchParams.get("type") as "PERCENT" | "EUROS" || "PERCENT";
    const commercialText = searchParams.get("text") || "";
    ...
    const getCleanAmenities = () => {
        ...
        return combined.filter((item, index) => combined.findIndex(t => t.trim().toLowerCase() === item.trim().toLowerCase()) === index);
    };

    const CompactDiagnostic = ({ currentLetter, title, subtitle, icon, labels }: any) => {
        ...
        return (
            <div className="bg-white rounded-[24px] border border-zinc-200/70 shadow-sm overflow-hidden">
                <div className="px-5 pt-4 pb-3 flex items-center gap-2">
                    <span className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${COLORS.secondary}15`, color: COLORS.primary }}>{icon}</span>
                    ...
                    <div className="flex-1">
                        <div className="flex items-end h-10 gap-1 mb-2">
                            {["A", "B", "C", "D", "E", "F", "G"].map((letter) => (
                                <div key={letter} className="flex-1 rounded-t-md transition-all" style={{ backgroundColor: DPE_COLORS[letter], opacity: letter === currentLetter ? 1 : 0.3, height: letter === currentLetter ? '100%' : '50%' }} />
                            ))}
                        </div>
                        <div className="flex justify-between text-[8px] font-black text-zinc-400">
                            {["A", "B", "C", "D", "E", "F", "G"].map((letter) => (
                                <span key={letter} className="flex-1 text-center" style={{ color: letter === currentLetter ? DPE_COLORS[letter] : undefined, fontWeight: letter === currentLetter ? 900 : 700 }}>{letter}</span>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="px-5 py-2.5 text-center" style={{ backgroundColor: `${color}12` }}>
                    <p className="text-[9px] font-black uppercase tracking-wider" style={{ color: color }}>{labels[currentLetter] || "—"}</p>
                </div>
            </div>
        );
    };
    ...
    if (loading) return (
        <div className="min-h-screen bg-[#faf8f6] flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-zinc-500 text-xs uppercase tracking-[0.3em] font-bold">Chargement du dossier…</p>
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen font-sans pb-16" style={{ backgroundColor: '#e8e8ec' }}>
            ...
            <div className="plaquette-container flex flex-col items-center pt-8">
                ...
                <div className="print-page w-[210mm] h-[297mm] relative flex flex-col overflow-hidden" style={{ backgroundColor: COLORS.ivory }}>
                    <div className="relative w-full h-[58%] overflow-hidden shrink-0">
                        <img src={baseData.mainPhoto} className="w-full h-full object-cover" alt="Vue principale"/>
                        ...
                        <div className="absolute bottom-8 left-10 right-10 text-white">
                            <p className="text-[10px] uppercase tracking-[0.25em] font-black mb-2 flex items-center gap-2" style={{ color: COLORS.secondary, textShadow: '0 2px 8px rgba(0,0,0,0.6)' }}><Home size={13}/> Exclusivité</p>
                            <h1 className="font-serif text-5xl font-bold tracking-tight mb-2 uppercase" style={{ textShadow: '0 4px 20px rgba(0,0,0,0.8), 0 2px 8px rgba(0,0,0,0.6)' }}>{getDynamicTitle()}</h1>
                            <p className="text-base font-medium flex items-center gap-2" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}><MapPin size={16} style={{ color: COLORS.secondary }}/> {baseData.propertyAddress}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 px-10 pt-8 pb-8 relative flex flex-col">
                        ...
                        <div className="relative bg-white rounded-[32px] border border-white shadow-[0_30px_60px_-15px_rgba(138,14,1,0.35)] overflow-hidden shrink-0">
                            ...
                            <div className="px-8 py-7 flex items-end justify-between gap-6">
                                <div>
                                    <p className="font-black tracking-tighter leading-none text-zinc-900 text-[5.5rem] font-mono-num">{formatPrice(sellingPriceFAI)}<span className="ml-2 text-4xl font-black align-top" style={{ color: COLORS.primary }}>€</span></p>
                                    <p className="text-[11px] font-bold text-zinc-500 mt-3 leading-relaxed">Prix net vendeur : <span className="font-black text-zinc-700">{formatPrice(netVendeur)} €</span><br/><span className="opacity-80">+ {feeType === "PERCENT" ? agencyFees + "%" : formatPrice(feeAmount)+" €"} d'honoraires à charge acquéreur</span></p>
                                </div>
                ...
                {/* PAGE 2 */}
                <div className="print-page-wrapper">
                <div className="print-page w-[210mm] h-[297mm] relative flex flex-col p-12" style={{ backgroundColor: COLORS.ivory }}>
                    <div className="flex justify-between items-end border-b border-zinc-300 pb-4 mb-6 shrink-0">
                        <div><p className="text-[9px] uppercase tracking-[0.3em] font-black mb-1" style={{ color: COLORS.primary }}>Dossier technique</p><h2 className="font-serif text-2xl font-black text-zinc-900">{getDynamicTitle()}</h2><p className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 mt-0.5">{baseData.propertyAddress}</p></div><img src="/logo-patrim.png" alt="PATRIM" className="h-10 object-contain shrink-0"/>
                    </div>
                    ...
                    <div className="shrink-0"><div className="flex items-center gap-3 mb-3"><span className="text-[9px] uppercase tracking-[0.3em] font-black" style={{ color: COLORS.primary }}>Accès Interactif</span><div className="h-px flex-1 bg-zinc-300"/></div>
                        <div className="grid grid-cols-2 gap-5">
                            <a href={photosUrl} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-[24px] p-4 border border-zinc-200/70 shadow-sm relative overflow-hidden group hover:border-[#d35f52] transition-colors cursor-pointer">
                                <div className="absolute inset-x-0 top-0 h-1" style={{ background: `linear-gradient(90deg, ${COLORS.primary}, ${COLORS.secondary})` }}/>
                                <div className="flex items-center gap-4 pt-1"><div className="bg-zinc-50 p-2 rounded-xl border border-zinc-100 shrink-0 group-hover:scale-105 transition-transform">{domain && <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(photosUrl)}&margin=1&color=8a0e01`} alt="QR Photos" className="w-16 h-16"/>}</div><div><div className="flex items-center gap-1.5 mb-1"><ImageIcon size={10} style={{ color: COLORS.primary }}/><span className="text-[8px] uppercase tracking-[0.2em] font-black" style={{ color: COLORS.primary }}>01</span></div><h3 className="text-sm font-black text-zinc-900 mb-0.5 tracking-tight">Galerie Photos</h3><p className="text-[9px] text-zinc-500 font-medium leading-tight">Album HD du bien</p><div className="flex items-center gap-1 mt-1.5 text-[9px] font-bold" style={{ color: COLORS.primary }}><MousePointerClick size={9}/> Toucher l'encart</div></div></div>
                            </a>
                            <a href={simulationUrl} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-[24px] p-4 border border-zinc-200/70 shadow-sm relative overflow-hidden group hover:border-[#8a0e01] transition-colors cursor-pointer">
                                <div className="absolute inset-x-0 top-0 h-1" style={{ background: `linear-gradient(90deg, ${COLORS.primary}, ${COLORS.secondary})` }}/>
                                <div className="flex items-center gap-4 pt-1"><div className="bg-zinc-50 p-2 rounded-xl border border-zinc-100 shrink-0 group-hover:scale-105 transition-transform">{domain && <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(simulationUrl)}&margin=1&color=8a0e01`} alt="QR Simulation" className="w-16 h-16"/>}</div><div><div className="flex items-center gap-1.5 mb-1"><Calculator size={10} style={{ color: COLORS.primary }}/><span className="text-[8px] uppercase tracking-[0.2em] font-black" style={{ color: COLORS.primary }}>02</span></div><h3 className="text-sm font-black text-zinc-900 mb-0.5 tracking-tight">Simulateur de Prêt</h3><p className="text-[9px] text-zinc-500 font-medium leading-tight">Mensualité & rentabilité</p><div className="flex items-center gap-1 mt-1.5 text-[9px] font-bold" style={{ color: COLORS.primary }}><MousePointerClick size={9}/> Toucher l'encart</div></div></div>
                            </a>
                        </div>
                    </div>
