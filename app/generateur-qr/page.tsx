...
import { 
    ...
    X, Plus, Loader2, Download, ArrowLeft, Banknote, Home, Layers
} from "lucide-react";
...
export default function GenerateurQR() {
    ...
    const handleGenerate = async () => {
        ...
        const payloadJson = {
            ...
            propertyAddress: address, // Sauvegardé ici pour le simulateur/galerie
            propertyType,
            rooms: Number(rooms),
    ...
    const drawCardOnContext = async (
        ...
        // Rappel Adresse/Quartier (Petit gris en bas à droite)
        if (address) {
            ctx.textAlign = "right";
        ...
        // 4. Chargement et Dessin du QR Code (A gauche)
        const img = new Image();
    ...
    // --- TÉLÉCHARGEMENT D'UNE CARTE SEULE ---
    const downloadSingleQR = async (qrDataUrl: string, type: "SIMULATION" | "GALERIE") => {
        ...
        link.download = `Patrim_QR Express_${type}.png`;
        link.href = canvas.toDataURL("image/png");
        link.click();
    };

    // --- TÉLÉCHARGEMENT DE L'IMAGE COMBINÉE (Les 2 côte à côte) ---
    const downloadCombinedQR = async () => {
        if (!generatedId) return;
        const simuUrl = `${domain}/simulation/${generatedId}`;
        const galerieUrl = `${domain}/galerie/${generatedId}`;

        // Canevas large pour contenir les deux cartes (2000px) + un petit espace (50px)
        const spacer = 50;
        const canvas = document.createElement("canvas");
        ...
        // Fond global légèrement gris pour faire ressortir les cartes blanches
        ctx.fillStyle = "#f4f4f5";
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        // Dessiner la première carte (Simulation) à gauche
        await drawCardOnContext(ctx, 0, 0, "SIMULATION", simuUrl);

        // Dessiner la deuxième carte (Galerie) à droite (après le spacer)
        await drawCardOnContext(ctx, 1000 + spacer, 0, "GALERIE", galerieUrl);

        // Téléchargement
        const link = document.createElement("a");
        link.download = `Patrim_QR Express_COMBINE.png`;
        link.href = canvas.toDataURL("image/png");
        link.click();
    };
            ...
            <div className="max-w-5xl mx-auto px-6 mt-10">
                {!generatedId ? (
                    ...
                    <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-500 space-y-12">
                        ...
                        {/* Preview visuelle avec Adresse intégrée */}
                        ...
                        {/* SECTION ULTIME : TÉLÉCHARGER LE PACK COMBINÉ */}
