...
export default function SimulateurAcquereur() {
    ...
    // --- INPUTS UTILISATEUR ---
    const [offerPrice, setOfferPrice] = useState<number>(0); // Nouveau : prix d'offre du client
    ...
    // Pré-remplir le prix d'offre avec le prix de présentation une fois chargé
    useEffect(() => {
    ...
    // --- CALCULS FINANCIERS (basés sur le prix d'offre) ---
    ...
    // Valeur indicative du bien à l'horizon sélectionné (pour la petite phrase de contexte)
    const indicativeFutureValue = price * Math.pow(1 + APPRECIATION_RATE, selectedYear);

    // --- CALCULS FISCAUX LOCATIF (Nu vs LMNP) ---
    const annualRent = expectedRent * 12;
    const annualCharges = monthlyCharges * 12;

    const nuTaxableIncome = annualRent * 0.70; // Abattement micro-foncier 30%
    const nuTax = Math.max(0, nuTaxableIncome * (DEFAULT_TMI + SOCIAL_CHARGES));

    const lmnpTaxableIncome = annualRent * 0.50; // Abattement micro-BIC 50%
    ...
    return (
        <div className="min-h-screen bg-[#faf8f6] text-zinc-900 font-sans pb-20 selection:bg-[#d35f52]/30 relative overflow-hidden">
            ...
            <div className="md:hidden">
                ...
                {/* Cartouche prix de présentation (référence officielle FAI) */}
            ...
            {/* ============================================================
                HERO DESKTOP
                ============================================================ */}
            <div className="hidden md:block relative w-full h-[55vh] min-h-[480px] overflow-hidden">
                <img src={data.mainPhoto} className="w-full h-full object-cover" alt="Bien" />
                <div className="absolute inset-0" style={{ background: `linear-gradient(180deg, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.4) 60%, rgba(0,0,0,0.8) 100%)` }} />
                ...
                <div className="absolute bottom-32 left-8 right-8 text-white max-w-3xl">
                    <p className="text-xs uppercase tracking-[0.3em] font-black mb-2 flex items-center gap-2" style={{ color: COLORS.secondary, textShadow: '0 2px 6px rgba(0,0,0,0.6)' }}>
                        <TrendingUp size={14}/> Simulateur Financier Premium
                    </p>
                    <h1 className="font-serif text-6xl font-bold leading-tight" style={{ textShadow: '0 4px 16px rgba(0,0,0,0.7)' }}>
                        {data.propertyType} {data.rooms > 0 && `T${data.rooms}`}
                    </h1>
                </div>

                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/3 z-20">
            ...
            <div className="px-5 mt-6 md:-mt-16 relative z-10 space-y-5 max-w-2xl mx-auto">
                ...
                {/* ---------- 1. DONNÉES DU PROJET (inclut le prix d'offre) ---------- */}
                <div className="bg-white/80 backdrop-blur-2xl p-7 rounded-[32px] border border-white shadow-[0_20px_60px_-15px_rgba(138,14,1,0.15)] space-y-6">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            {/* Pastille d'écart — visible uniquement si différent du prix affiché */}
                        ...
                        {/* Garde-fou bienveillant si offre < -10% */}
                ...
                {/* ============================================================
                    3. GRAPHIQUE AMORTISSEMENT — Version simple et lisible
                       ▸ Une seule courbe : capital restant dû (descendante)
                       ▸ KPI héros : "Capital remboursé"
                       ▸ Scrubbing + jalons 5/10/15/20 ans
                       ▸ Phrase de contexte indicative sur la valorisation
                    ============================================================ */}
                {loanAmount > 0 && (
                    <div className="bg-white/80 backdrop-blur-2xl p-7 rounded-[32px] border border-white shadow-[0_20px_60px_-15px_rgba(138,14,1,0.15)]">
                        ...
                        <div className="mb-6 p-5 rounded-2xl border border-zinc-200/70 bg-gradient-to-br from-zinc-50 to-white shadow-inner">
                            ...
                            {/* KPI héros : Capital remboursé à cette année */}
                        ...
                        {/* ====== LE GRAPHIQUE SVG — Une seule courbe ====== */}
                        <div
                        ...
                        >
                            <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-56 overflow-visible" preserveAspectRatio="none">
                                ...
                                {MILESTONES.filter(m => m <= safeDuration).map(m => (
                                    <circle
                                        key={`ms-${m}`}
                                        cx={(m / safeDuration) * svgWidth}
                                        cy={svgPadding.top + chartHeight}
                                        r="3"
                                        fill="#fff"
                                ...
                                {/* Point sur la courbe — halo + anneau rouge */}
                        ...
                        <p className="text-center text-[9px] text-zinc-400 uppercase tracking-widest font-bold mt-5 mb-4 flex items-center justify-center gap-2">
                        ...
                        {/* ============================================================
                            Phrase de contexte indicative sur la valorisation
                            Discrète, en bas, sans graphique complexe
                            ============================================================ */}
                ...
                {/* ============================================================
                    4. OPTION LOCATIVE — WATERFALL + LMNP
                    ============================================================ */}
...
/* ============================================================
   ...
   Affiche Loyer → −Charges → −Mensualité → Cash-Flow
   Chaque barre est proportionnelle au loyer de référence.
   ============================================================ */
