...
import {
  ...
  Target, Shield, Eye, BookOpen, ArrowLeft,
} from "lucide-react";
...
function MiniViz({ type }: { type: string }) {
  ...
  if (type === "rsi") return (
    <svg {...c}>
      ...
      <line x1={20} y1={32} x2={300} y2={32} stroke="#dc262618" strokeDasharray="4 4" />
      <line x1={20} y1={98} x2={300} y2={98} stroke="#05966918" strokeDasharray="4 4" />
      <text x={304} y={36} fill="#dc2626" fontSize={9} fontWeight={600}>70</text>
      <text x={304} y={102} fill="#059669" fontSize={9} fontWeight={600}>30</text>
      <rect x={20} y={0} width={280} height={32} rx={0} fill="#dc262606" />
      <rect x={20} y={98} width={280} height={32} rx={0} fill="#05966906" />
      <polyline points="20,65 45,55 70,42 95,28 115,32 135,48 160,62 185,78 210,95 235,102 255,90 275,72 300,58" fill="none" stroke="#2563eb" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
      <circle cx={95} cy={28} r={4} fill="#dc2626" />
      <text x={75} y={22} fill="#dc2626" fontSize={8} fontWeight={600}>Surachat</text>
      <circle cx={235} cy={102} r={4} fill="#059669" />
      <text x={215} y={118} fill="#059669" fontSize={8} fontWeight={600}>Survente</text>
    </svg>
  );
...
export default function GuidePage() {
  ...
  const filtered = SECTIONS.filter(s => filter === "all" || s.category === filter);
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: "#9a98a8" }}>Guide interactif — Nexus Stocks</p>
              </div>
            </div>
            <p className="text-[15px] leading-relaxed" style={{ color: "#5a5a72" }}>
              Chaque indicateur est un outil. Seul, il est imprécis. Combinés intelligemment (confluence), ils deviennent puissants. Ce guide explique le rôle de chaque outil utilisé par Nexus Stocks.
            </p>
          </div>
          ...
          <div className="flex gap-2 mb-8 flex-wrap">
            {([["all", "Tout"], ["concept", "Concepts"], ["methode", "Méthodes"], ["indicateur", "Indicateurs"]] as const).map(([k, l]) => (
              <button key={k} onClick={() => { setFilter(k); setActiveId(null); }}
          ...
          <AnimatePresence mode="wait">
            {!active ? (
              ...
              <motion.div key="detail" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                ...
                <div className="flex justify-center py-2"><MiniViz type={active.visual} /></div>
                ...
                <div className="flex justify-between pt-4 border-t" style={{ borderColor: "#eae8e4" }}>
                  {(() => {
                    ...
                    return (
                      <>
                        {prev ? <button onClick={() => setActiveId(prev.id)} className="flex items-center gap-2 text-sm font-semibold" style={{ color: "#5a5a72" }}><ChevronLeft size={16} /> {prev.title}</button> : <div />}
                        {next && <button onClick={() => setActiveId(next.id)} className="flex items-center gap-2 text-sm font-semibold" style={{ color: "#2563eb" }}>{next.title} <ChevronRight size={16} /></button>}
                      </>
                    );
                  })()}
                </div>
