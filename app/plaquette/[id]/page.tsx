...
export default function PlaquetteManager() {
    ...
    // URLs interactives pour les QR Codes
    ...
    const CompactDiagnostic = ({
        ...
        currentLetter: string; title: string; subtitle: string; icon: React.ReactNode; labels: Record<string, string>;
    }) => {
        ...
        return (
            <div className="bg-white rounded-[24px] border border-zinc-200/70 shadow-sm overflow-hidden">
                    ...
                    <div className="flex-1">
                        <div className="flex items-end h-10 gap-1 mb-2">
                            {["A", "B", "C", "D", "E", "F", "G"].map((letter) => {
                                ...
                                return (
                                    <div key={letter} className="flex-1 rounded-t-md transition-all" style={{ backgroundColor: DPE_COLORS[letter], opacity: isCurrent ? 1 : 0.3, height: isCurrent ? '100%' : '50%' }} />
                                );
                            })}
                        </div>
                ...
                <div className="px-5 py-2.5 text-center" style={{ backgroundColor: `${color}12` }}>
                    <p className="text-[9px] font-black uppercase tracking-wider" style={{ color: color }}>{label}</p>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="min-h-screen font-sans pb-32" style={{ backgroundColor: '#e8e8ec' }}>
            <style jsx global>{`
                ...
                /* RÈGLES STRICTES POUR L'IMPRESSION */
                @media print {
                    @page { size: A4 portrait; margin: 0; }
                    body {
                        margin: 0 !important; padding: 0 !important;
                        -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; background-color: white !important;
                    }
                    .print-hidden { display: none !important; }
                    
                    .plaquette-container { display: flex !important; flex-direction: column !important; gap: 0 !important; margin: 0 !important; padding: 0 !important; background: white !important; }
                    ...
                    .print-page-wrapper:last-child { page-break-after: auto !important; }
            ...
            <div className="hidden md:flex fixed bottom-10 left-1/2 -translate-x-1/2 text-white px-8 py-4 rounded-full items-center gap-5 shadow-2xl z-50 print-hidden border bg-[#0a0a0c]/95 backdrop-blur-md">
                ...
                <Button onClick={() => window.print()} className="rounded-full px-7 h-10 font-bold text-sm bg-gradient-to-r from-[#8a0e01] to-[#d35f52]">
                    <Printer size={15} className="mr-2"/> Enregistrer en PDF
                </Button>
            </div>
            ...
            <div className="md:hidden fixed bottom-5 left-3 right-3 text-white px-3 py-2.5 rounded-full flex items-center gap-2 shadow-2xl z-50 print-hidden border border-white/10 bg-[#0a0a0c]/95 backdrop-blur-md">
                <button onClick={() => router.back()} className="text-zinc-400 hover:text-white p-2.5 rounded-full shrink-0" aria-label="Retour"><ArrowLeft size={16}/></button>
                <div className="w-px h-5 bg-white/10"></div>
                <button onClick={() => window.print()} className="flex-1 flex items-center justify-center gap-2 px-4 h-10 font-bold text-xs rounded-full bg-gradient-to-r from-[#8a0e01] to-[#d35f52] shrink-0">
                    <Printer size={14}/><span>Enregistrer PDF</span>
                </button>
            </div>
                ...
                <div className="print-page w-[210mm] h-[297mm] shadow-2xl relative flex flex-col overflow-hidden" style={{ backgroundColor: COLORS.ivory }}>
                    ...
                    <div className="relative w-full h-[58%] overflow-hidden shrink-0">
                        ...
                        <div className="absolute bottom-8 left-10 right-10 text-white">
                            ...
                            <h1 className="font-serif text-5xl font-bold tracking-tight mb-2 uppercase text-white" style={{ textShadow: '0 4px 20px rgba(0,0,0,0.8), 0 2px 8px rgba(0,0,0,0.6)' }}>{getDynamicTitle()}</h1>
                            <p className="text-base font-medium flex items-center gap-2 text-white" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}><MapPin size={16} style={{ color: COLORS.secondary }}/> {baseData.propertyAddress}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 px-10 pt-8 pb-8 relative flex flex-col">
                        ...
                        <div className="relative bg-white rounded-[32px] border border-white shadow-[0_30px_60px_-15px_rgba(138,14,1,0.35)] overflow-hidden shrink-0">
                            <div className="px-8 py-3.5 flex items-center justify-between" style={{ background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.secondary})` }}>
                                <span className="text-[11px] uppercase tracking-[0.3em] font-black text-white">Prix de Présentation</span>
                            ...
                            <div className="px-8 py-7 flex items-end justify-between gap-6">
                                <div>
                                    <p className="font-black tracking-tighter leading-none text-zinc-900 text-[5.5rem] font-mono-num">
                                        {formatPrice(sellingPriceFAI)}<span className="ml-2 text-4xl font-black align-top" style={{ color: COLORS.primary }}>€</span>
                                    </p>
                                    <p className="text-[11px] font-bold text-zinc-500 mt-3 leading-relaxed">
                                        Prix net vendeur : <span className="font-black text-zinc-700">{formatPrice(netVendeur)} €</span><br/>
                                        <span className="opacity-80">+ {feeType === "PERCENT" ? agencyFees + "%" : formatPrice(feeAmount)+" €"} d'honoraires à charge acquéreur</span>
                                    </p>
                                </div>
                                <div className="text-right border-l border-zinc-200 pl-6">
                                    ...
                                    <p className="font-black text-3xl font-mono-num" style={{ color: COLORS.primary }}>{formatPrice(pricePerSqm)}</p>
                        ...
                        <div className="grid grid-cols-4 gap-3 mt-6 shrink-0">
                            {[
                                ...
                                <div key={idx} className="bg-white rounded-2xl p-3.5 border border-zinc-200/70 shadow-sm flex flex-col items-center text-center">
                                    <div className="w-8 h-8 rounded-xl mb-2 flex items-center justify-center text-white shadow-sm" style={{ background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.secondary})` }}>{item.icon}</div>
                                    <p className="text-[8px] uppercase font-bold tracking-[0.2em] text-zinc-400 mb-0.5">{item.label}</p>
                                    <p className="text-lg font-black text-zinc-900 font-mono-num leading-tight">{item.value}{item.unit && <span className="text-[10px] font-bold text-zinc-400 ml-0.5">{item.unit}</span>}</p>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex-1 w-full h-full flex flex-col px-10 py-10">
                        ...
                        <div className="grid grid-cols-5 gap-5 mb-5 shrink-0">
                            ...
                            <div className="col-span-2 bg-white rounded-[24px] p-5 border border-zinc-200/70 shadow-sm">
                                <h3 className="text-[10px] uppercase tracking-[0.25em] font-black mb-3 flex items-center gap-2 text-zinc-700"><Layers size={12}/> Équipements</h3>
                                <div className="flex flex-wrap gap-1.5">
                                    {getCleanAmenities().map((item: any, i: number) => (
                                        <span key={i} className="text-[9px] font-bold px-2.5 py-1 rounded-lg border uppercase tracking-tight" style={{ backgroundColor: `${COLORS.secondary}08`, borderColor: `${COLORS.secondary}25`, color: COLORS.gray }}>{item}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[24px] p-5 border border-zinc-200/70 shadow-sm mb-5 shrink-0">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="flex justify-between items-center py-1.5 border-b border-zinc-100"><span className="text-xs text-zinc-500 font-bold">Copropriété</span><span className="text-base font-black font-mono-num" style={{ color: COLORS.gray }}>{baseData.isCopropriete ? formatPrice(baseData.coproFees * 12) : "N/A"}{baseData.isCopropriete && <span className="text-[10px] opacity-60"> €</span>}</span></div>
                            </div>
                        </div>
                        ...
                        <div className="mb-5 shrink-0">
                            ...
                            <div className="grid grid-cols-2 gap-5">
                                ...
                                <CompactDiagnostic currentLetter={baseData.ges} title="GES — Émissions" subtitle="kg CO₂/m²/an" icon={<CloudFog size={11}/>} labels={GES_LABELS}/>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-5">
                                <a href={photosUrl} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-[24px] p-4 border border-zinc-200/70 shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-4 pt-1">
                                        <div className="bg-zinc-50 p-2 rounded-xl border border-zinc-100 shrink-0">{domain && <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(photosUrl)}&margin=1&color=8a0e01`} alt="QR Photos" className="w-16 h-16"/>}</div>
                                        <div>
                                            <div className="flex items-center gap-1.5 mb-1"><ImageIcon size={10} style={{ color: COLORS.primary }}/><span className="text-[8px] uppercase tracking-[0.2em] font-black" style={{ color: COLORS.primary }}>01</span></div>
                                            ...
                                            <div className="flex items-center gap-1 mt-1.5 text-[9px] font-bold" style={{ color: COLORS.primary }}><MousePointerClick size={9}/> Scanner ou cliquer</div>
                                        </div>
                                    </div>
                                </a>
                                ...
                                <a href={simulationUrl} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-[24px] p-4 border border-zinc-200/70 shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="absolute inset-x-0 top-0 h-1" style={{ background: `linear-gradient(90deg, ${COLORS.primary}, ${COLORS.secondary})` }}/>
                                    <div className="flex items-center gap-4 pt-1">
                                        <div className="bg-zinc-50 p-2 rounded-xl border border-zinc-100 shrink-0">{domain && <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(simulationUrl)}&margin=1&color=8a0e01`} alt="QR Simulation" className="w-16 h-16"/>}</div>
                                        <div>
                                            <div className="flex items-center gap-1.5 mb-1"><Calculator size={10} style={{ color: COLORS.primary }}/><span className="text-[8px] uppercase tracking-[0.2em] font-black" style={{ color: COLORS.primary }}>02</span></div>
                        ...
                        <div className="mt-auto pt-4 border-t border-zinc-300 flex items-center justify-between w-full">
                            ...
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border" style={{ backgroundColor: `${COLORS.secondary}08`, borderColor: `${COLORS.secondary}25` }}>
                                <ArrowUpRight size={10} style={{ color: COLORS.primary }}/><span className="text-[8px] uppercase tracking-widest font-black" style={{ color: COLORS.primary }}>Document non contractuel</span>
                            </div>
                        </div>
